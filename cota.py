...
DATA_MARCA_DAGUA_API = "2023-12-31"
FUNDOS = {
    CNPJ_MINAS_FIA: {
        "nome": "MINAS FIA",
...
def get_ibov_acumulado(data_inicio: str, data_fim: str) -> float:
    try:
        ...
        if isinstance(close.columns if hasattr(close, 'columns') else None, pd.MultiIndex):
            close = close.iloc[:, 0]
        close = close.squeeze()  # garante Series 1D independentemente da versão
                ...
                if cnpj_selecionado == CNPJ_MINAS_FIA:
                    ...
                    if not df_b100.empty:
                        ...
                        if not {"Data", "Minas", "IBOV", "DI1F29"}.issubset(set(df_b100.columns)):
                            ...
                            if not df_ytd.empty:
                                minas_base = df_ytd['Minas'].iloc[0]
                                ibov_base = df_ytd['IBOV'].iloc[0]
                                ...
                                df_plot_ibov_ytd = df_ytd.melt("Data",
                                                               value_vars=["Minas_YTD", "IBOV_YTD"],
                                                               var_name="Série", value_name="Valor").dropna()

                                max_valor_1_ytd = df_plot_ibov_ytd["Valor"].max()
                                max_y_1_ytd = math.ceil(max_valor_1_ytd / 10) * 10 + 10
                                min_y_1_ytd = df_plot_ibov_ytd["Valor"].min() * 0.95

                                chart_1_ytd = (
                                    alt.Chart(df_plot_ibov_ytd)
                                    .mark_line(strokeWidth=2)
                                    .encode(
                                        x=alt.X("Data:T", title="Data"),
                                        y=alt.Y("Valor:Q",
                                                title="Índice (base=100)",
                                                scale=alt.Scale(domain=[min_y_1_ytd, max_y_1_ytd])),
                                        color=alt.Color("Série:N",
                                                        scale=alt.Scale(
                                                            domain=["Minas_YTD", "IBOV_YTD"],
                                                            range=["#d62728", "#1f77b4"]
                                                        )),
                                        tooltip=[
                                            alt.Tooltip("Data:T", title="Data"),
                                ...
                                chart_minas_ytd = (
                                    ...
                                    .encode(
                                        ...
                                        y=alt.Y("Minas_YTD:Q",
                                                title="Índice Base 100",
                                                scale=alt.Scale(domain=domain_minas_ytd)),
                                        tooltip=[
                                            alt.Tooltip("Data:T", title="Data"),
                                            alt.Tooltip("Minas_YTD:Q", title="Minas", format=".1f")
                                        ]
                                    )
                                )
                                ...
                                chart_di_ytd = (
                                    ...
                                    .encode(
                                        ...
                                        y=alt.Y("DI1F29:Q",
                                                title="Taxa (%)",
                                                axis=alt.Axis(orient="right", format=".1%"),
                                                scale=alt.Scale(domain=domain_di_ytd)),
                                        tooltip=[
                                            alt.Tooltip("Data:T", title="Data"),
                                            alt.Tooltip("DI1F29:Q", title="DI1F29", format=".1%")
                                        ]
                                ...
                                st.altair_chart(chart_2_ytd, use_container_width=True)
